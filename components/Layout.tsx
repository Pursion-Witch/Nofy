...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  onStatusChange, latestCriticalLog 
}) => {
  ...
  // Trigger popup when a new critical log arrives
  useEffect(() => {
    if (latestCriticalLog) {
      setShowCriticalPopup(true);
      const timer = setTimeout(() => setShowCriticalPopup(false), 8000); // Auto hide after 8s
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 font-sans overflow-x-hidden">
      ...
      {/* CRITICAL INCIDENT POPUP (Top Half Restriction) */}
      {showCriticalPopup && latestCriticalLog && (
        <div className="fixed inset-x-0 top-0 h-[50vh] z-[100] bg-rose-950/95 backdrop-blur-md border-b-4 border-rose-500 shadow-2xl animate-in slide-in-from-top duration-500 flex flex-col items-center justify-center p-6 text-center">
            ...
            <audio autoPlay src="data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YU..." /> {/* Simulated Beep - shortened */}
        </div>
      )}
      ...
      <div className="sticky top-0 z-50 bg-slate-900 shadow-xl">
        ...
        {/* ALERT BAR: DASHBOARD - Unified Command Feed */}
        <div className={`border-b border-slate-800 relative transition-all duration-300 bg-slate-950 ${alertsExpanded ? 'h-auto py-4' : 'h-8 py-1 overflow-hidden'}`}>
           <div className="max-w-7xl mx-auto px-4 flex justify-between items-start">
              ...
              <button 
                 ...
                 className="ml-4 p-1 hover:bg-slate-800 rounded text-slate-400"
              >
        ...
        <header className="bg-slate-900 border-b border-slate-800 px-4 py-3">
          <div className="max-w-md md:max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
               {/* Traffic Light Status for Terminal */}
               ...
               {/* Chat Button (Opens Main Overlay) */}
               <button 
                  onClick={() => setShowFullChat(true)}
                  className="relative p-2 hover:bg-indigo-900/30 rounded-full transition-colors group"
                  title="Team Chat"
               >
                  <MessageSquare className="w-5 h-5 text-indigo-400 group-hover:text-indigo-300" />
                  {unreadCount > 0 && (
                     <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-rose-500 text-white text-[8px] flex items-center justify-center rounded-full border border-slate-900"></span>
                  )}
               </button>
                                ...
                                {['ONLINE', 'BUSY', 'BREAK', 'OFFLINE'].map((status) => (
                                   <button 
                                      ...
                                      className={`px-2 py-1.5 rounded-lg text-[10px] font-bold border transition-all flex items-center justify-center gap-1 ${
                                         user.status === status 
                                         ? status === 'ONLINE' ? 'bg-emerald-600 border-emerald-500 text-white' :
                                           status === 'BUSY' ? 'bg-amber-600 border-amber-500 text-white' :
                                           status === 'BREAK' ? 'bg-blue-600 border-blue-500 text-white' : 'bg-slate-600 border-slate-500 text-white'
