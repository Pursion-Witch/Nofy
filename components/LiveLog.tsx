...
import { Building2, Maximize2, X, AlertTriangle, Shield, Info, Activity, MessageCircle, Clock, Send, CheckCircle2 } from 'lucide-react';
...
export const LiveLog: React.FC<LiveLogProps> = ({ logs, currentUser }) => {
  ...
  const submitUpdate = async () => {
      if (!activeUpdateLog || !updateContent.trim() || !currentUser) {
          console.warn("Update check failed:", { activeUpdateLog, hasContent: !!updateContent.trim(), currentUser });
          return;
      }
      ...
      try {
          ...
          // Re-fetch log with new updates for modal
  ...
  const LogContent = () => (
    <div className="space-y-3">
        ...
        {logs.map((log) => {
          ...
          return (
            <div key={log.id} className={`p-4 rounded-xl border-l-4 ${config.color} animate-in fade-in slide-in-from-right-2 duration-300 relative`}>
              ...
              <div className="w-full max-w-lg bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh]">
                  ...
                  <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                      <div>
                          ...
                          <p className="text-[10px] text-slate-500 font-mono mt-0.5 truncate max-w-[250px]">Ref: {activeUpdateLog.id}</p>
                      </div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={4}
                                    placeholder="Provide the latest status or resolution details..."
