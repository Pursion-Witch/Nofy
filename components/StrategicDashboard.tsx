...
import { UIConceptGenerator } from './UIConceptGenerator';
...
export const StrategicOverview: React.FC<StrategicOverviewProps> = ({ 
  ...
  return (
    ...
    <div className="flex flex-col gap-3 lg:gap-4 h-auto lg:h-[calc(100vh-220px)] w-full pb-20 lg:pb-0">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-4">
          <div className="lg:col-span-3 flex flex-col gap-2 lg:gap-3 min-h-0 order-2 lg:order-1">
             <button onClick={() => setShowAssetStudio(true)} className="bg-slate-800 rounded-xl p-4 border border-indigo-500/30 shadow-lg flex items-center justify-between hover:border-indigo-500 transition-all group overflow-hidden relative">
                <div className="absolute top-0 right-0 w-16 h-16 bg-indigo-500/10 blur-2xl group-hover:bg-indigo-500/20 transition-all"></div>
                <div className="flex items-center gap-3 relative z-10">
                   <div className="p-2 bg-indigo-600 rounded-lg shadow-lg"><Sparkles className="w-4 h-4 text-white" /></div>
                   <div className="text-left">
                      <div className="text-xs font-black text-white uppercase tracking-tight">NOFY Asset Studio</div>
                      <div className="text-[10px] text-slate-400">Generate 4K Concept PNG</div>
                   </div>
                </div>
             ...
             <div className="flex-1 flex flex-col gap-2 lg:gap-3 lg:overflow-y-auto custom-scrollbar">
                {userDept === Department.AIRLINE_OPS && (
                    <button id="card-airline-comm" onClick={() => setShowAirlineOps(true)} className="bg-gradient-to-r from-indigo-900 to-slate-800 rounded-xl border border-indigo-500 p-3 lg:p-4 hover:brightness-110 transition-all group relative overflow-hidden text-left flex items-center justify-between shadow-lg">
                        <div>
                            <div className="flex items-center gap-2 text-indigo-300 font-bold text-[10px] lg:text-xs uppercase mb-0.5"><Plane className="w-3 h-3 lg:w-4 lg:h-4" /> Command</div>
                ...
                {userDept === Department.AOCC && (
                    <button id="card-aocc-liaison" onClick={() => setShowAOCCLiaison(true)} className="bg-gradient-to-r from-amber-900 to-slate-800 rounded-xl border border-amber-500 p-3 lg:p-4 hover:brightness-110 transition-all group relative overflow-hidden text-left flex items-center justify-between shadow-lg">
                        <div>
                            <div className="flex items-center gap-2 text-amber-300 font-bold text-[10px] lg:text-xs uppercase mb-0.5"><RadioTower className="w-3 h-3 lg:w-4 lg:h-4" /> Control</div>
                ...
                <button onClick={handleOpenCebuConnects} className="bg-slate-800 rounded-xl border border-slate-700 p-3 lg:p-4 hover:border-sky-500 transition-all group relative overflow-hidden text-left flex items-center justify-between shadow-sm">
                    ...
                    <div>
                        <div className="flex items-center gap-2 text-sky-400 font-bold text-[10px] lg:text-xs uppercase mb-0.5"><ArrowRightLeft className="w-3 h-3 lg:w-4 lg:h-4" /> Transfer</div>
                ...
                <button onClick={handleOpenCebuPlus} className="bg-slate-800 rounded-xl border border-slate-700 p-3 lg:p-4 hover:border-indigo-500 transition-all group relative overflow-hidden text-left flex items-center justify-between shadow-sm">
                    ...
                    <div>
                        <div className="flex items-center gap-2 text-indigo-400 font-bold text-[10px] lg:text-xs uppercase mb-0.5"><Ship className="w-3 h-3 lg:w-4 lg:h-4" /> Intermodal</div>
                ...
                <button onClick={handleOpenCebBalik} className="bg-slate-800 rounded-xl border border-slate-700 p-3 lg:p-4 hover:border-amber-500 transition-all group relative overflow-hidden text-left flex items-center justify-between shadow-sm">
                    ...
                    <div>
                        <div className="flex items-center gap-2 text-amber-400 font-bold text-[10px] lg:text-xs uppercase mb-0.5"><Briefcase className="w-3 h-3 lg:w-4 lg:h-4" /> OFW Hub</div>
          ...
          <div className="lg:col-span-6 flex flex-col min-h-0 order-1 lg:order-2 h-[220px] lg:h-auto">
             <div className="bg-slate-800 rounded-xl border border-slate-700 p-1 lg:p-4 h-full shadow-lg flex flex-col relative overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-2 px-3 pt-3 lg:px-0 lg:pt-0 shrink-0">
                   <div><h3 className="text-white font-bold flex items-center gap-2 text-sm lg:text-base"><Activity className="w-4 h-4 lg:w-5 lg:h-5 text-indigo-400" /> Passenger Flow</h3></div>
                ...
                <div className="flex-grow w-full min-h-0">
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={flowData} margin={{ top: 10, right: 0, left: -25, bottom: 0 }}>
                            ...
                            <CartesianGrid strokeDasharray="3 3" stroke="#334155" vertical={false} /><XAxis dataKey="time" stroke="#64748b" fontSize={10} tickLine={false} axisLine={false} /><YAxis stroke="#64748b" fontSize={10} tickLine={false} axisLine={false} /><Tooltip contentStyle={{ backgroundColor: '#1e293b', borderColor: '#334155', borderRadius: '8px', fontSize: '12px', padding: '8px' }} itemStyle={{ color: '#e2e8f0' }} /><Area type="monotone" dataKey="inbound" stroke="#38bdf8" fillOpacity={1} fill="url(#colorIn)" strokeWidth={2} /><Area type="monotone" dataKey="outbound" stroke="#34d399" fillOpacity={1} fill="url(#colorOut)" strokeWidth={2} />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
             </div>
             ...
             <div className="bg-slate-800 rounded-xl border border-slate-700 h-full flex flex-col shadow-lg overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
                   {activeFeedTab === 'ALERTS' && (
                       criticalLogs.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-slate-500"><CheckCircle2 className="w-8 h-8 opacity-20 mb-2" /><span className="text-xs italic">All Clear</span></div> :
                   ...
                   {activeFeedTab === 'WATCHLIST' && (
                       delayedFlights.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-slate-500"><Plane className="w-8 h-8 opacity-20 mb-2" /><span className="text-xs italic">No Delays</span></div> :
    ...
    {showAssetStudio && <UIConceptGenerator onClose={() => setShowAssetStudio(false)} />}
