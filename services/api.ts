...
const DB_KEY = 'NOFY_VIRTUAL_DB_V3';
...
class BackendService {
  ...
  private load(): AirportDatabase {
    ...
    if (saved) {
      try {
        ...
        console.error("DB Corruption detected, resetting to initial data", e);
      }
    }
  ...
  private save(event: BackendEvent) {
    ...
    this.notify(event);
    this.broadcast.postMessage(event); // Sync other tabs
  }
  ...
  private handleExternalEvent(event: BackendEvent) {
    // Basic reactive logic for incoming broadcast events
    switch(event.type) {
